...
function PageContent() {
  ...
  const handleGenerate = async () => {
    ...
    if (!isPro && usageCount >= 5) {
      setError('Has alcanzado el límite diario de 5 respuestas gratuitas.');
      return;
    }
            ...
            <div className="flex items-center gap-4">
              ...
              {!isPro && (
                <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full border border-border-color shadow-sm">
                  ...
                  <span className="text-xs font-bold text-text-secondary uppercase">{Math.max(0, 5 - usageCount)}/5 gratis</span>
                </div>
              )}
            </div>
        </div>
